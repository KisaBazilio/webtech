...
### HTML тэги
Структура документа в языке HTML описывается при помощи специальных тэгов. С полным списков тэгов и их 
назначением можно ознакомиться  [здесь](https://html5book.ru/html-tags/)

Необазательно запоминать все тэги, потому что многие из них используются крайне редко, для начала хватит 
следующих основных тэгов:
...
Любой HTML-документ обязан содержать минимальный набор тэгов для корректной работы в браузере.
...
автоматически вставить в документ короткой командой). Например в редакторе `VS Code` в пустом файле с расширением `.html` можно
ввести восклицательный знак `!` и нажать на `tab` на клавиатуре, чтобы заполнить пустой документ следующим образом:
...
Почитайте про семантическую верстку [здесь](https://htmlacademy.ru/blog/articles/semantics)
...
* `git add .` - необходимо находиться в папке с репозиторием, индексирует измененные файлы и добавляет их к 
 следующему коммиту (вместо "." можно указывать название конкретного файла чтобы добавить только его, `git add .` добавляет к следующему коммиту все обновленные файлы)
...
GitHub предоставляет возможность бесплатно публиковать ваши статичные сайты (состоящи только
...
![](assets\pages.PNG)
3. Ждем (процесс публикации сайта обычно занимает 1-5 минут), переодически обновляя страницу
4. Когда сайт опубликуется в том же разделе вы увидете ссылку на сайт
   ![](assets\site.PNG)

**Это и следующие задания будут считаться полностью выполненными только в случае размещения их
